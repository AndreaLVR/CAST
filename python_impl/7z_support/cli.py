...
def print_usage():
    ...
    print("    --dict-size <SIZE>   : 7-Zip LZMA Dictionary Size (Default: 128MB)")
...
def do_compress(input_path, output_path, chunk_size=None, dict_size=None, verify=False):
    ...
    # Default to 128MB logic
    final_dict_size = dict_size if dict_size else 128 * 1024 * 1024
    ...
    dict_str = format_bytes(final_dict_size)
    ...
    try:
        with open(input_path, "rb") as f_in, open(output_path, "wb") as f_out:
            while True:
                ...
                res = compressor.compress(chunk_data, dict_size=final_dict_size)

                # Unpacking with support for the new 'strategy_name' (5th element)
                strategy_name = "Unknown"
                if isinstance(res, tuple):
                    if len(res) >= 5:
                        c_reg, c_ids, c_vars, id_flag, strategy_name = res[:5]
                    elif len(res) >= 4:
                        c_reg, c_ids, c_vars, id_flag = res[:4]
                    else:
                        raise ValueError("Unexpected tuple length from cast.py")
                else:
                    raise ValueError("Unexpected compressor output type")

                # Print Strategy info only on first chunk to avoid spam
                if chunk_idx == 1:
                    print(f"\n       [i] Active Strategy: {strategy_name}")
                    print(
                        f"       Processing Chunk #{chunk_idx} ({format_bytes(chunk_len)})... ",
                        end="",
                        flush=True,
                    )
